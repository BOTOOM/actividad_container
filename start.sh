...
SCHEMASPY_SCHEMA=$SCHEMASPY_SCHEMA DB_BUILD_USER=$DB_BUILD_USER DB_BUILD_NAME=$DB_BUILD_NAME DB_BUILD_PASS=$DB_BUILD_PASS CLIENT_PORT=$CLIENT_PORT PROXY=$http_proxy USER_ID=$UID CLIENT_CLONE_INFO=$CLIENT_CLONE_INFO 
CLIENT_BRANCH_TARGET=$CLIENT_BRANCH_TARGET 
ADMISIONES_CRUD_GET_INFO=$ADMISIONES_CRUD_GET_INFO ADMISIONES_CRUD_BRANCH_TARGET=$ADMISIONES_CRUD_BRANCH_TARGET ADMISIONES_CRUD_PORT=$ADMISIONES_CRUD_PORT docker-compose down
...
if [ -f workspace/ ]; then
    ...
    mkdir -p workspace/go/src/github.com/BOTOOM
    echo "Carpetas creadas..."
fi
mkdir -p database
mkdir -p apache2/sites-enabled
echo "Construyendo Contendor..."
SCHEMASPY_SCHEMA=$SCHEMASPY_SCHEMA DB_BUILD_USER=$DB_BUILD_USER DB_BUILD_NAME=$DB_BUILD_NAME DB_BUILD_PASS=$DB_BUILD_PASS CLIENT_PORT=$CLIENT_PORT PROXY=$http_proxy USER_ID=$UID CLIENT_CLONE_INFO=$CLIENT_CLONE_INFO CLIENT_BRANCH_TARGET=$CLIENT_BRANCH_TARGET ADMISIONES_CRUD_GET_INFO=$ADMISIONES_CRUD_GET_INFO ADMISIONES_CRUD_BRANCH_TARGET=$ADMISIONES_CRUD_BRANCH_TARGET ADMISIONES_CRUD_PORT=$ADMISIONES_CRUD_PORT docker-compose build
